...
async def main():
    ...
    # Create a perlin noise generator (using 1 octave; adjust as needed)
    noise_gen = PerlinNoise(octaves=1, seed=random.randint(0, 100))
    ...
    for i in range((WIDTH // rez) + 1):
        ...
        for j in range((HEIGHT // rez) + 1):
            # Using a random initialization for the grid values (0 or 1)
    ...
    while running:
        ...
        # Update the grid using perlin noise
        for i in range((WIDTH // rez) + 1):
            for j in range((HEIGHT // rez) + 1):
                zoom = 0.1
                zoom_diff = 0.01
                ...
                # The noise generator expects a list of coordinates.
                value = noise_gen([ (i * zoom) + dt * x_speed, (j * zoom) - dt * y_speed ])
                # Clamp and normalize input to between 0 and 1
                grid[i][j] = (value + 1) / 2

        # Draw the grid
        for i in range((WIDTH // rez) + 1):
            for j in range((HEIGHT // rez) + 1):
                # for each dot, draw a circle of darkness depending on the noise value
                red = 255 * grid[i][j]
                green = 255 * grid[i][j]
                blue = 255 * grid[i][j]
        ...
        for i in range((WIDTH // rez)):
            for j in range((HEIGHT // rez)):
                ...
                # define midpoints (not used explicitly here, but kept for reference)
                a = (i * rez + (rez // 2), j * rez)        # Top middle
                b = (i * rez + rez, j * rez + (rez // 2))    # Right middle
                c = (i * rez + (rez // 2), j * rez + rez)    # Bottom middle
                d = (i * rez, j * rez + (rez // 2))          # Left middle
